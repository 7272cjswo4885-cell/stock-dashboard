...
const MACRO_DEFS = {
  심리: [
    {id:"fear_greed",  label:"공포탐욕지수",        unit:"/100",range:[0,100],  good:[25,50],   icon:"🧠", val:38},
    {id:"vix",         label:"VIX 변동성지수",       unit:"",    range:[10,60],  good:[13,25],   icon:"📈", val:24.3},
    {id:"aaii_bull",   label:"AAII 강세 비율",       unit:"%",   range:[10,60],  good:[25,38],   icon:"📊", val:33.6},
  ],
  유동성: [
  ...
  금리통화: [
    {id:"yield_10y",   label:"10년물 국채금리",       unit:"%",   range:[0,6],    good:[2.0,4.5], icon:"🏛️", val:4.35},
  ...
  경기사이클: [
    ...
    {id:"jobless_claims",label:"신규 실업수당 청구",   unit:"만건", range:[100,300], good:[150,220],  icon:"📋", val:202},
    {id:"jolts",       label:"구인건수 (JOLTS)",        unit:"만건", range:[400,1200],good:[700,900], icon:"💼", val:688},
  ],
  시장구조: [
    {id:"new_lows",    label:"52주 신저가 비율",       unit:"%",   range:[0,50],   good:[0,15],    icon:"📉", val:5},
...
function resolveMacroDefs(md = {}) {
  const applyLiveValue = def => ({
    ...def,
    val: Object.prototype.hasOwnProperty.call(md, def.id) ? md[def.id] : undefined,
  });
  return {
    심리: MACRO_DEFS.심리.map(applyLiveValue),
    유동성: MACRO_DEFS.유동성.map(applyLiveValue),
    금리통화: MACRO_DEFS.금리통화.map(applyLiveValue),
    경기사이클: MACRO_DEFS.경기사이클.map(applyLiveValue),
    시장구조: MACRO_DEFS.시장구조.map(applyLiveValue),
  };
}
...
function average(arr) {
  const nums = Array.isArray(arr) ? arr.filter(v => typeof v === "number" && !Number.isNaN(v)) : [];
  return nums.length ? nums.reduce((a, b) => a + b, 0) / nums.length : undefined;
}

function round(value, decimals = 1) {
  return typeof value === "number" ? parseFloat(value.toFixed(decimals)) : undefined;
}

function ema(values, period) {
  if (!Array.isArray(values) || values.length < period) return undefined;
  const slice = values.slice(values.length - period - 1);
  let emaValue = average(slice.slice(0, period));
  if (emaValue === undefined) return undefined;
  const k = 2 / (period + 1);
  for (let i = period; i < slice.length; i += 1) {
    emaValue = slice[i] * k + emaValue * (1 - k);
  }
  return emaValue;
}

function calcRSI(closes, period = 14) {
  if (!Array.isArray(closes) || closes.length <= period) return undefined;
...
function calcMACD(closes) {
  if (!Array.isArray(closes) || closes.length < 26) return undefined;
...
function calcBB(closes) {
  if (!Array.isArray(closes) || closes.length < 20) return undefined;
  const last20 = closes.slice(-20);
  const middle = average(last20);
  if (middle === undefined) return undefined;
  const variance = average(last20.map(x => Math.pow(x - middle, 2)));
  const std = Math.sqrt(variance);
  return { upper: middle + 2 * std, lower: middle - 2 * std, middle };
}

function calcRSvsBenchmark(closes, benchmark) {
  if (!Array.isArray(closes) || !Array.isArray(benchmark) || closes.length < 21 || benchmark.length < 21) return undefined;
  const symbolReturn = closes[closes.length - 1] / closes[closes.length - 21] - 1;
  const benchmarkReturn = benchmark[benchmark.length - 1] / benchmark[benchmark.length - 21] - 1;
  return round((symbolReturn - benchmarkReturn) * 100, 1);
}

function buildStockFromQuote(symbol, quote, benchmarkHistory) {
  const closes = quote.history.map(h => h.price).filter(v => typeof v === "number" && !Number.isNaN(v));
  const volumes = quote.history.map(h => h.volume).filter(v => typeof v === "number" && !Number.isNaN(v));
  if (!closes.length) return mockStock(symbol);

  const price = typeof quote.price === "number" ? quote.price : closes[closes.length - 1];
  const high52 = typeof quote.high52 === "number" ? quote.high52 : Math.max(...closes);
  const low52 = typeof quote.low52 === "number" ? quote.low52 : Math.min(...closes);
  const ma50 = closes.length >= 50 ? round(average(closes.slice(-50)), 2) : round(average(closes), 2);
  const ma200 = closes.length >= 200 ? round(average(closes.slice(-200)), 2) : round(average(closes), 2);
  const ma200_30d = closes.length >= 230 ? round(average(closes.slice(-230, -30)), 2) : undefined;
  const ma200_slope = ma200_30d ? round(((ma200 - ma200_30d) / ma200_30d) * 100, 1) : undefined;
  const rsi = calcRSI(closes);
  const macd = calcMACD(closes);
  const bb = calcBB(closes);
  const bb_pos = bb && bb.upper !== bb.lower ? round(((price - bb.lower) / (bb.upper - bb.lower)) * 100, 1) : undefined;
  const week52_pos = high52 > low52 ? round(((price - low52) / (high52 - low52)) * 100, 1) : undefined;
  const mdd = high52 ? round(((price - high52) / high52) * 100, 1) : undefined;
  const avgVolume = quote.avgVolume || (volumes.length ? round(average(volumes.slice(-20))) : undefined);
  const vol_ratio = quote.volume && avgVolume ? round(quote.volume / avgVolume, 2) : undefined;
  const vol_signal = vol_ratio > 1.5 && price < ma50 ? "공포 매도 감지" : vol_ratio > 1.3 ? "거래량 급증" : vol_ratio < 0.7 ? "거래량 감소" : "정상";
  const benchmarkCloses = Array.isArray(benchmarkHistory) ? benchmarkHistory : undefined;
  const rs_vs_qqqm = calcRSvsBenchmark(closes, benchmarkCloses);

  // 펀더멘털 데이터 추가 (실제로는 API에서 가져와야 하지만 임시로 mock)
  const mockFundamentals = mockStock(symbol);
  const peAvg = mockFundamentals.peAvg;
  const pe = mockFundamentals.pe;
  const eps_growth = mockFundamentals.eps_growth;
  const peg = mockFundamentals.peg;
  const analyst_target = mockFundamentals.analyst_target;
  const insider = mockFundamentals.insider;
...
function genData(stocks, quotes = {}) {
  const benchmarkHistory = quotes.QQQM?.history?.map(h => h.price).filter(v => typeof v === "number" && !Number.isNaN(v));
  const s = {};
  stocks.forEach(st => {
    const quote = quotes[st.id];
    if (quote && Array.isArray(quote.history) && quote.history.length > 1) {
      s[st.id] = buildStockFromQuote(st.id, quote, benchmarkHistory);
    } else {
      s[st.id] = mockStock(st.id);
    }
  });
  return s;
}
...
function calcScore(sd, w, md = {}) {
  const weights = w || INVEST_MODES.dca.weights;
  const has = value => typeof value === "number";
  const fear_greed = md.fear_greed;
  const vix = md.vix;
  const aaii_bull = md.aaii_bull;
  const m2_growth = md.m2_growth;
  const yield_10y = md.yield_10y;
  const dxy = md.dxy;
  const oil_wti = md.oil_wti;
  const fed_bs = md.fed_bs;
  const credit_spread = md.credit_spread;
  const yield_gap = md.yield_gap;
  const new_lows = md.new_lows;
  const ism_pmi = md.ism_pmi;
  const yield_spread = md.yield_spread;
  const cpi = md.cpi;
  const unemployment = md.unemployment;
  ...
  let sent=50;
  if(has(fear_greed)){
    if(fear_greed<25)sent+=18; else if(fear_greed<40)sent+=9; else if(fear_greed>75)sent-=18;
  }
  if(has(vix)){
    if(vix>30)sent+=14; else if(vix>25)sent+=7; else if(vix<15)sent-=7;
  }
  if(has(aaii_bull)){
    if(aaii_bull<25)sent+=10; else if(aaii_bull<32)sent+=5; else if(aaii_bull>50)sent-=10;
  }
  if(sd.insider===2)sent+=12; else if(sd.insider===1)sent+=6;

  let macro=50;
  if(has(m2_growth)){
    if(m2_growth>3&&m2_growth<9)macro+=10; else if(m2_growth<0)macro-=14;
  }
  if(has(yield_10y)){
    if(yield_10y>4.5)macro-=12; else if(yield_10y<3)macro+=8;
  }
  if(has(dxy)){
    if(dxy>107)macro-=9; else if(dxy<97)macro+=7;
  }
  if(has(oil_wti)){
    if(oil_wti>90)macro-=7; else if(oil_wti<65)macro-=5;
  }
  if(has(fed_bs)){
    if(fed_bs>8)macro+=10; else if(fed_bs>7)macro+=5; else if(fed_bs<5)macro-=8;
  }
  if(has(credit_spread)){
    if(credit_spread<1.5)macro+=8; else if(credit_spread>3)macro-=15; else if(credit_spread>2)macro-=7;
  }
  if(has(yield_gap)){
    if(yield_gap>2)macro+=10; else if(yield_gap>0.5)macro+=5; else if(yield_gap<0)macro-=12;
  }
  if(has(new_lows)){
    if(new_lows>30)macro+=10; else if(new_lows>20)macro+=5; else if(new_lows<5)macro-=5;
  }
  if(has(ism_pmi)){
    if(ism_pmi>55)macro+=8; else if(ism_pmi>50)macro+=4; else if(ism_pmi<45)macro-=10;
  }
  if(has(yield_spread)){
    if(yield_spread<-0.5)macro-=10; else if(yield_spread>0.5)macro+=6;
  }
  if(has(cpi)){
    if(cpi<2.5)macro+=8; else if(cpi>4)macro-=10;
  }
  if(has(unemployment)){
    if(unemployment<4)macro+=5; else if(unemployment>5)macro-=8;
  }
...
// 거시경제 신호등 패널 (종합현황 탭용)
function MacroSnapshot({investMode, md}){
  ...
  const allInds=[...macroDefs.심리,...macroDefs.유동성,...macroDefs.금리통화,...macroDefs.경기사이클,...macroDefs.시장구조];
    ...
    <div style={{marginTop:10,padding:"14px",borderRadius:12,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:7}}>
        {items.map(item=>{
          ...
          const displayValue = item.val == null ? "—" : `${item.val}${item.unit}`;
          return(
            <div key={item.id} style={{padding:"8px 9px",background:"rgba(0,0,0,0.25)",borderRadius:8,borderLeft:`2px solid ${c}`}}>
              ...
              <div style={{fontSize:15,fontWeight:900,fontFamily:"'Space Mono',monospace",color:c}}>{displayValue}</div>
            </div>
          );
        })}
      </div>
...
function MacroSummary({macroDefs}){
  const allInds=[...macroDefs.심리,...macroDefs.유동성,...macroDefs.금리통화,...macroDefs.경기사이클,...macroDefs.시장구조];
  const summaryItems=[
...
export default function App() {
  ...
  useEffect(()=>{
    setDataTime(new Date().toLocaleString("ko-KR",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}));
  },[]);
  ...
  // ✅ useMemo로 캐싱 — stocks 또는 quotes가 바뀔 때 재계산
  const stockData=useMemo(()=>genData(stocks, quotes),[stocks, quotes]);

  const stockInfo=stocks.find(s=>s.id===sel)||stocks[0];
  const sd=stockData[sel]||stockData[stocks[0]?.id];
  const scores=sd?calcScore(sd,mode.weights,md):{total:50,tech:50,sent:50,macro:50,fund:50};
      ...
      <div style={{maxWidth:920,margin:"0 auto",padding:"0 24px"}}>
        ...
        {tab==="overview"&&sd&&(
          <div style={{animation:"slideIn 0.35s ease"}}>
            ...
            {/* ✅ 거시경제 신호등 패널 */}
            <MacroSnapshot investMode={investMode} md={md}/>
          </div>
        )}
          ...
          <div style={{animation:"slideIn 0.35s ease",marginTop:14}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,gap:8,flexWrap:"wrap"}}>
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {Object.keys(macroDefs).map(cat=>(
                  <button key={cat} onClick={()=>{setMacroTab(cat);setKeyOnly(false);}} style={{padding:"6px 13px",borderRadius:7,fontSize:11,fontWeight:600,border:`1px solid ${macroTab===cat&&!keyOnly?"rgba(0,212,255,0.5)":"rgba(255,255,255,0.1)"}`,background:macroTab===cat&&!keyOnly?"rgba(0,212,255,0.1)":"rgba(255,255,255,0.03)",color:macroTab===cat&&!keyOnly?"#00d4ff":"rgba(255,255,255,0.4)",cursor:"pointer",transition:"all 0.2s"}}>
                    {cat} <span style={{fontSize:9,opacity:0.6}}>({macroDefs[cat].length})</span>
                  </button>
                ))}
              </div>
              {/* ✅ 핵심 지표 필터 버튼 */}
              <button onClick={()=>setKeyOnly(o=>!o)} style={{padding:"6px 13px",borderRadius:7,fontSize:11,fontWeight:700,border:`1px solid ${keyOnly?mode.color:"rgba(255,255,255,0.15)"}`,background:keyOnly?`rgba(${rgb(mode.color)},0.15)`:"rgba(255,255,255,0.03)",color:keyOnly?mode.color:"rgba(255,255,255,0.45)",cursor:"pointer",transition:"all 0.2s",whiteSpace:"nowrap",flexShrink:0}}>
                ⭐ 핵심만 보기 {keyOnly&&`(${KEY_INDICATORS[investMode]?.filter(id=>[...macroDefs.심리,...macroDefs.유동성,...macroDefs.금리통화,...macroDefs.경기사이클,...macroDefs.시장구조].find(d=>d.id===id)).length}개)`}
              </button>
            </div>
            ...
            {keyOnly?(
              ...
              <div>
                {Object.entries(macroDefs).map(([cat,inds])=>{
              ...
              <div style={{display:"grid",gap:9}}>
                {macroDefs[macroTab].map(ind=><MacroCard key={ind.id} ind={ind} mode={investMode}/>)}
              </div>
            )}

            {/* ✅ 거시환경 종합 판단 */}
            <MacroSummary macroDefs={macroDefs}/>
          </div>
        )}
